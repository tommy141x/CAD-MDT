...
$unit_callsign = "";
$unit_name = "";
$queryquery = $database->query("SELECT unit_name FROM mdt_units WHERE unit_type='DISPATCH' AND steam_id='".$steamprofile['steamid']."'");
...
<div class='modal fade' id='login' tabindex='-1' role='dialog' aria-labelledby='New911CallLabel' aria-hidden='true'>
                <div class='modal-dialog' role='document'>
                    ...
                    <form class='form-inline' method='post' action='dispatch-login.php'>
                      ...
                      <input name='name' type='text' class='form-control mb-1 mr-sm-2' id='inlineFormInputName2' placeholder='Officer Name' size='25' required>
						...
						<input type='hidden' name='steam_id' value=".$steamprofile['steamid'].">
                          ...
                          ";
        if (!($final4res == "DISPATCH")) {
            echo "<code>You are currently logged into another panel.</code><button type='submit' class='btn btn-primary mr-2' action='dispatch-login.php' disabled>Login</button>";
        } else {
            echo "<button type='submit' class='btn btn-primary mr-2' action='dispatch-login.php'>Login</button>";
        }
        echo "
...
<html lang="en">
  <head>
    ...
    <title><?php echo $communityName;?> | Dispatch Panel </title>
    <!-- plugins:css -->
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
			        <div id="hTitle"></div>
			<div>
			<div style="display: inline-block;" id="acButton"></div>
            ...
            if (isset($_GET["callsign"])) {
                echo "
                <a href='dispatch-logout.php'><button type='button' class='btn btn-inverse-secondary btn-fw'>Logout</button></a>
			";
            }
            ?>
                    ...
                    <div class="modal fade" id="call-view" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-2" aria-hidden="true">
                      <div class="modal-dialog modal-lg" role="document">
                            <div class="fetched-data"></div>
							                    ...
							                    <div class="modal fade" id="unit-view" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-4" aria-hidden="true">
							                      <div class="modal-dialog" role="document">
							                            <div class="fetched-unit-data"></div>
							                      </div>
							                    </div>
														                    <div class="modal fade" id="app-view" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-6" aria-hidden="true">
														                      <div class="modal-dialog" role="document">
														                            <div class="fetched-app-data"></div>
														                      </div>
														                    </div>
...
return event.keyCode != 13;
    ...
    $('#call-view').on('show.bs.modal', function (e) {
        ...
        $.ajax({
	     ...
	     $('#unit-view').on('show.bs.modal', function (e) {
	         var id = $(e.relatedTarget).data('unitid');
	         $.ajax({
	             type : 'post',
	 	     ...
	 	     $('#app-view').on('show.bs.modal', function (e) {
	 	         var id = $(e.relatedTarget).data('appid');
	 	         $.ajax({
	 	             type : 'post',
	 	             url : 'fetchApp.php', //Here you will fetch records
	 	             data :  { id: id},
	 	             success : function(data){
	 	             $('.fetched-app-data').html(data);//Show fetched data from database
	 	             }
	 	         });
	 	      });
});
	...
	<script language="javascript" type="text/javascript">
		$('#acButton').load('backend.php?value=acButton');
		$('#calls').load('backend.php?value=calls');
		$('#units').load('backend.php?value=units');
